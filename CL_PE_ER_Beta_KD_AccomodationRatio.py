...
parser.add_argument('--lwf_lamda', type=float, default=1.6)
...
# Statistics
statistics_path = './statistics/OnlineProtoNet_DeepConvLSTM_{}_baseClasses_{}_newClasses_{}_percentage_{}_online_epochs_{}_WeightedUpdate_{}_not_all_buffer_classes_{}_random_stream_{}.pkl'.format(params.data, 
    params.baseClasses, params.newClasses, params.percentage, params.online_epochs, params.alpha, params.not_all_buffer_classes, params.random_stream)
forgetting_path = './forgetting_score/OnlineProtoNet_ER_Beta_DeepConvLSTM_{}_baseClasses_{}_percentage_{}_online_epochs_{}_WeightedUpdate_{}_contrastive_loss_{}.pkl'.format(params.data, params.baseClasses, params.percentage, params.online_epochs, params.alpha, params.contrastive_loss)
...
# query_set = [d for d in baseData['data']]
# query_labels_set = [l for l in y_train]
# query_map_labels = [np.argmax(l) for l in y_train]
...
#        query_set = torch.from_numpy(np.array(query_set)).float()
        ...
        print("Replay Buffer: ", np.shape(replay_set), np.shape(replay_map_labels), Counter(replay_map_labels))

        # query_set = np.append(saved_support_set, replay_set, axis=0)
        # query_map_labels = np.append(saved_map_labels, replay_map_labels, axis=0)


        replay_set = torch.from_numpy(np.array(replay_set)).float()
        replay_map_labels = tf.keras.utils.to_categorical(replay_map_labels, num_classes=baseClassesNb + newClassesNb, dtype='int32')
        replay_map_labels = torch.from_numpy(np.array(replay_map_labels)).float()

        query_set = torch.cat((support_set,replay_set),0)
        query_map_labels = torch.cat((map_labels, replay_map_labels), 0)
        print("QUERY SET + REPLAY: ", Counter(np.argmax(query_map_labels.data.cpu().numpy(), axis=1)))

        # query_set = torch.from_numpy(np.array(query_set)).float()
        # query_map_labels = tf.keras.utils.to_categorical(query_map_labels, num_classes=baseClassesNb + newClassesNb, dtype='int32')
        # query_map_labels = torch.from_numpy(np.array(query_map_labels)).float()
        replay_set = replay_set.cuda()
        replay_map_labels = replay_map_labels.cuda()
        query_set = query_set.cuda()
        query_map_labels = query_map_labels.cuda()

        query_h = model.extractor.init_hidden(len(query_set))        
        ...
        p, val_h,_ = model.forward_online(support_set,map_labels, query_set, val_h,query_h,lwf=True)
        stored_model.eval()
        if (not params.random_stream and counter > 1) or (params.random_stream and counter > N):
            old_classes = torch.tensor(list((stored_model.memory.prototypes.keys()))).cuda()
            #new_classes = [c for c in np.sort(np.unique(np.argmax(map_labels))) if c not in old_classes]
            with torch.no_grad():
                pre_p,_ = stored_model.forward_online(support_set, map_labels, query_set, val_h, query_h,lwf=True)

            p_modified = modify_new_logits(p, pre_p, old_classes, params.beta)

            log_p = F.softmax(-p_modified,dim=1)
            loss_hard_targets = F.binary_cross_entropy(log_p, query_map_labels)
            print("loss_hard_target {}".format(loss_hard_targets))
            loss = loss_hard_targets
            stored_model = copy.deepcopy(model)

        else:
            log_p = F.softmax(-p,dim=1)
            loss = F.binary_cross_entropy(log_p, query_map_labels)
        if params.contrastive_loss:
            _,_,z_query = model.extractor(query_set,query_h, query_set.size(0))
            loss += ContrastiveLoss(z_query, query_map_labels) 
        #running_loss += loss
        loss.backward()
        optimizer.step()
        ...
        for j in range(1, online_epochs):
            ...
            query_set1, query_map_labels1 = utils.shuffle(query_set,query_map_labels, random_state=j)

            #sys.exit()
            query_set1 = query_set1.cuda()
            query_map_labels1 = query_map_labels1.cuda()

            query_h = tuple([each.data for each in query_h])
            optimizer.zero_grad()


            p, query_h = model.forward_online_QUERY(query_set1, query_h, lfw=True)
            stored_model.eval()
            if (not params.random_stream and counter > 1) or (params.random_stream and counter > N):
                    #old_classes = list(model.memory.prototypes.keys())
                    #new_classes = [c for c in np.sort(np.unique(np.argmax(map_labels))) if c not in old_classes]
                with torch.no_grad():
                    pre_p,_,_ = stored_model.forward_online_QUERY(query_set1, query_h, lwf=True)

                p_modified = modify_new_logits(p, pre_p, old_classes, params.beta)

                log_p = F.softmax(-p_modified,dim=1)
                loss_hard_targets = F.binary_cross_entropy(log_p, query_map_labels1)    
                loss = loss_hard_targets
            else:
                log_p = F.softmax(-p,dim=1)
                loss = F.binary_cross_entropy(log_p, query_map_labels1)
            if params.contrastive_loss:
                _,_,z_query = model.extractor(query_set1,query_h, query_set1.size(0))
                loss += ContrastiveLoss(z_query, query_map_labels1) 
            #running_loss += loss
            loss.backward()
            optimizer.step()
        #model.online_update_prototypes(support_set,map_labels, val_h)
        ...
        # update prototypes of classes using weighted sum of old prototypes and prototypes from replay data (high momentum update set alpha = 0.99)
        model.eval()
        replay_h = model.extractor.init_hidden(len(replay_set))
        replay_h = tuple([each.data for each in replay_h])

        if len(replay_set) > 0:
            model.prototype_update_momentum(replay_set, replay_map_labels, params.alpha, replay_h)

        # query_set = list(query_set.data.cpu().numpy())
        # query_map_labels = copy.deepcopy(saved_map_labels)
        support_set = []
        labels_set = []
        map_labels = []
        ...
        with torch.no_grad():
            ...
            for d in test_loader:
                inputs, labels = d
                h = model.extractor.init_hidden(len(inputs))

                labels = np.argmax(labels.data.cpu().numpy(),axis=1)
                labels = tf.keras.utils.to_categorical(labels,num_classes=baseClassesNb+newClassesNb, dtype='int32') #max(int(max(np.array(labels))),max(model.memory.prototypes.keys()))+1,dtype='int32')
                labels = torch.from_numpy(labels).float()
                inputs = inputs.cuda()
                labels = labels.cuda()
                ...
                log_p, h = model.forward_online(inputs,labels,inputs,h,h)
            ...
            for d in train_loader:
                ...
                _,h,embeddings = model.extractor(inputs, h, len(inputs))
                embeddings_list['embeddings'].extend(embeddings.data.cpu().numpy())
            ...
            trainLoss = {'BaseTrainloss': epoch_loss}
            baseTrainF1 = {'BaseTrain_f1': fscore}
            #trainLoss = {'Trainloss': loss}
            ...
            accuracy = metrics.accuracy_score(true_test_oo, test_oo)
            precision, recall, fscore,_ = metrics.precision_recall_fscore_support(true_test_oo, test_oo, labels=np.unique(true_test_oo), average='macro')
            _,_,fscore_array,_ = metrics.precision_recall_fscore_support(true_test_oo, test_oo, average=None)
            
            for class_k in range(len(fscore_array[np.unique(true_test_oo)])):
                f1_score_t[class_k] = fscore_array[class_k]
                max_f1_score[class_k] = max(f1_score_t[class_k], max_f1_score[class_k])
             #import pdb; pdb.set_trace()
            ...
            forgetting_score = {'Forgetting Score': calculate_forgetting(f1_score_t, max_f1_score)}
            forgetting_container.append(iteration, forgetting_score, data_type='ForgettingScore')
            print('Forgetting Score: {}'.format(forgetting_score))
            ...
            test_newClasses_output = []
            true_test_newClasses_output = []
            ...
            if newClassesNb > 1:
                for d in test_newClasses_loader:
                    ...
                    test_newClasses_output.append(log_p.data.cpu().numpy())
                    true_test_newClasses_output.append(labels.data.cpu().numpy())
                    test_output.append(log_p.data.cpu().numpy())
                    true_test_output.append(labels.data.cpu().numpy())
                    n_test_steps += 1 

                test_newClasses_oo = np.argmax(np.vstack(test_newClasses_output), axis = 1)
                true_test_newClasses_oo = np.argmax(np.vstack(true_test_newClasses_output), axis = 1)

                test_oo = np.argmax(np.vstack(test_output), axis = 1)
                true_test_oo = np.argmax(np.vstack(true_test_output), axis = 1)

                accuracy = metrics.accuracy_score(true_test_newClasses_oo, test_newClasses_oo)
                precision, recall, fscore,_ = metrics.precision_recall_fscore_support(true_test_newClasses_oo, test_newClasses_oo, labels=np.unique(true_test_newClasses_oo), average='macro')
                try:
                    auc_test = metrics.roc_auc_score(np.vstack(true_test_newClasses_output), np.vstack(test_newClasses_output),labels=np.unique(true_test_newClasses_oo), average='macro')
                except ValueError:
                    auc_test = None
                print('Test loss: {}'.format(running_test_loss / n_test_steps))
                print('TEST New Classes average_precision: {}'.format(precision))
                print('TEST New Classes average f1: {}'.format(fscore))
                print('TEST New Classes average recall: {}'.format(recall))
                print('TEST New Classes auc: {}'.format(accuracy))
                ...
                try:
                    ...
                    auc_test = None
                print('Test loss: {}'.format(running_test_loss / n_test_steps))
                print('TEST All Classes average_precision: {}'.format(precision))
                print('TEST All Classes average f1: {}'.format(fscore))
                print('TEST All Classes average recall: {}'.format(recall))
                print('TEST All Classes auc: {}'.format(accuracy))


                testLoss = {'Testloss_NewClasses': running_test_loss / n_test_steps}
                statistics_container.append(iteration, testLoss, data_type='Testloss_NewClasses')
                test_f1 = {'newClasses_test_f1':fscore}
                statistics_container.append(iteration, test_f1, data_type='newClasses_test_f1')

        statistics_container.dump()
        forgetting_container.dump()
        ...
        replay_buffer.update((np.array(saved_support_set), np.array(saved_map_labels)))   
    counter += 1
    n_steps += 1

C = confusion_matrix(true_test_oo, test_oo,labels = list(mapping.values()))
plt.figure(figsize=(10,10))
plot_confusion_matrix(C, class_list=list(mapping.keys()), normalize=True, title='Predicted Results')

if newClassesNb > 1:
    plt.figure(figsize=(10,10))
    C = confusion_matrix(true_test_newClasses_oo, test_newClasses_oo, labels = list(mapping.values()))
    plot_confusion_matrix(C, class_list=list(mapping.keys()), normalize=True, title='Predicted Results')
...
# for i in range(len(true_test_oo)):
#     labels[i] = list(mapping.keys())[true_test_oo[i]]
plotForgettingScore(forgetting_path)
print('FINAL Forgetting Score: {}'.format(forgetting_score))
    ...
    if k in baseCl:
        ...
        plt.plot(after_prot[k,0], after_prot[k,1], 'o',
                markerfacecolor=col, markeredgecolor=col,
                 marker=markers[k%len(markers)],markersize=7)        
        #add label
        plt.annotate(LABELS[list(mapping.keys())[k]], (after_prot[k,0], after_prot[k,1]),
                     horizontalalignment='center',
                     verticalalignment='center',
                     size=10, weight='bold',rotation=45,
                     color='k')
...
with open("./prototypes_json/Debugging_{}_Data_OfflineTraining_batchSize_{}_epochs_{}_n_support_{}_ShuffleTrue_RemShuffle_NewClass_{}_ModelAdaptation_ReplayData.json".format(percentage,BATCH_SIZE,n_epochs,n_support,newClassesNb), "w") as write_file:
    str_ = json.dumps(json_dict)
    write_file.write(str_)
