...
if params.DSoftmax:
    D_softmax = DSoftmaxLoss(torch.Tensor([0.5]).float(), baseClassesNb + newClassesNb)
...
np.random.seed(seed)
# Statistics
statistics_path = './statistics/OnlineProtoNet_ER_DeepConvLSTM_{}_baseClasses_{}_percentage_{}_online_epochs_{}_WeightedUpdate_{}_not_all_buffer_classes_{}_random_stream_{}.pkl'.format(params.data, 
...
forgetting_path = './forgetting_score/OnlineProtoNet_ER_DeepConvLSTM_{}_baseClasses_{}_percentage_{}_online_epochs_{}_WeightedUpdate_{}_contrastive_loss_{}.pkl'.format(params.data, params.baseClasses, params.percentage, params.online_epochs, params.alpha, params.contrastive_loss)
...
if params.contrastive_loss or params.contrastive_loss_offline:
    ContrastiveLoss = OnlineContrastiveLoss(model, PairSelector(balance=False), margin=params.margin)
    ContrastiveLossOffline = OnlineContrastiveLoss(model, PairSelector(balance=False), margin=1.)
...
for epoch in range(n_epochs):
    ...
    for d in train_loader:
        ...
        if params.contrastive_loss or params.contrastive_loss_offline:
            ...
            loss +=(1+params.beta)*ContrastiveLossOffline(z_query, y_query)
        if params.contrastive_loss_with_prototypes:
            _,_,z_query = model.extractor(x_query,query_h, x_query.size(0))
...
#STEP_SIZE = 100
#scheduler = StepLR(optimizer, step_size=STEP_SIZE, gamma=.1)
...
while not dataHandler.endOfStream():
    ...
    if not params.random_stream or counter % N == 0:
        # if n_steps % STEP_SIZE and n_steps >0 :
        #     scheduler.step()
...
#        query_set = torch.from_numpy(np.array(query_set)).float()
        ...
        log_p, val_h,dists = model.forward_online(support_set,map_labels, query_set, val_h,query_h)
        ...
        if not params.DSoftmax:
            ...
            loss = D_softmax(dists, query_map_labels, model)
        if params.contrastive_loss:
            _,_,z_query = model.extractor(query_set,query_h, query_set.size(0))
            loss +=(1+params.beta)*ContrastiveLoss(z_query, query_map_labels) 
        if params.contrastive_loss_with_prototypes:
            _,_,z_query = model.extractor(query_set,query_h, query_set.size(0))
            loss += ContrastiveLossWithPrototypes(z_query, map_labels, model)    
        #running_loss += loss
        loss.backward()
        optimizer.step()
        ...
        for j in range(1, online_epochs):
            ...
            log_p, query_h = model.forward_online_QUERY(query_set1, query_h)
            #embeddings_list['embeddings'].extend(embds.data.cpu().numpy())
            #embeddings_list['labels'].extend(map_labels.data.cpu().numpy())
            loss = F.binary_cross_entropy(log_p, query_map_labels1)
            if params.contrastive_loss:
                _,_,z_query = model.extractor(query_set1,query_h, query_set1.size(0))
                loss +=(1+params.beta)*ContrastiveLoss(z_query, query_map_labels1)  
            if params.contrastive_loss_with_prototypes:
                _,_,z_query = model.extractor(query_set1,query_h, query_set1.size(0))
                loss += ContrastiveLossWithPrototypes(z_query, query_map_labels1, model)      
            #running_loss += loss
            loss.backward()
            optimizer.step()
        #model.online_update_prototypes(support_set,map_labels, val_h)
        ...
        with torch.no_grad():
           ...
           #import pdb; pdb.set_trace()
            ...
            if newClassesNb > 1:
                ...
                testLoss = {'Testloss_AllClasses': running_test_loss / n_test_steps}
                statistics_container.append(iteration, testLoss, data_type='Testloss_AllClasses')
                test_f1 = {'allClasses_test_f1':fscore}
                statistics_container.append(iteration, test_f1, data_type='allClasses_test_f1')
