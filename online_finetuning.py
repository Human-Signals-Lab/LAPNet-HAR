...
while not dataHandler.endOfStream():
    ...
    if not params.random_stream or counter % N == 0:
        ...
        with torch.no_grad():
            ...
            trainLoss = {'BaseTrainloss': running_loss1 / n_steps}
            baseTrainF1 = {'BaseTrain_f1': fscore}
            #trainLoss = {'Trainloss': loss}
...
print(np.unique(test_oo), len(np.unique(test_oo)), np.unique(true_test_oo), len(np.unique(true_test_oo)))
#labels = np.argmax(embeddings_list['labels'],axis=1)
labels = []
for i in sorted(np.unique(test_oo)):
    labels.append(list(mapping.keys())[i])

plt.figure(figsize=(10,10))
plot_confusion_matrix(C, class_list=np.unique(true_test_oo), normalize=True, title='Old Classes Predicted Results')

if newClassesNb > 1:
    plt.figure(figsize=(10,10))
    C = confusion_matrix(true_test_newClasses_oo, test_newClasses_oo, labels = np.unique(true_test_newClasses_oo))
    plot_confusion_matrix(C, class_list=np.unique(true_test_newClasses_oo), normalize=True, title='New Classes Predicted Results')
...
with open("./prototypes_json/Debugging_{}_Data_OfflineTraining_batchSize_{}_epochs_{}_n_support_{}_ShuffleTrue_RemShuffle_NewClass_{}_ModelAdaptation_{}_OnlineEpochs.json".format(percentage,BATCH_SIZE,n_epochs,n_support,newClassesNb,online_epochs), "w") as write_file:
    str_ = json.dumps(json_dict)
    write_file.write(str_)
...
#ll = np.array(list(model.memory.prototypes.keys()), dtype=np.int32)
for k, col in zip([NewClasses[0]], colors[len(baseClasses[:3])+4:]):
