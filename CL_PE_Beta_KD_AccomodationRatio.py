...
parser.add_argument('--NIC', action='store_true', default=False)
...
forgetting_path = './forgetting_score/OnlineProtoNet_Beta_DeepConvLSTM_{}_baseClasses_{}_percentage_{}_online_epochs_{}_WeightedUpdate_{}_contrastive_loss_{}.pkl'.format(params.data, params.baseClasses, params.percentage, params.online_epochs, params.alpha, params.contrastive_loss)
...
while not dataHandler.endOfStream():
    ...
    if params.random_stream:
        ...
        if params.NIC:
            d, l = dataHandler.NIC_generation(N)
        else:
...
#        query_map_labels = query_map_labels.cuda()
        ...
        p, val_h,_ = model.forward_online(support_set,map_labels, support_set, val_h,query_h,lwf=True)
        if (not params.random_stream and counter > 1) or (params.random_stream and counter > N):
            ...
            with torch.no_grad():
                pre_p,_ = stored_model.forward_online(support_set, map_labels, support_set, val_h, query_h,lwf=True)

            p_modified = modify_new_logits(p, pre_p, old_classes, params.beta)

            log_p = F.softmax(-p_modified,dim=1)
            loss_hard_targets = F.binary_cross_entropy(log_p, map_labels)

            loss = loss_hard_targets
            stored_model = copy.deepcopy(stored_model)
        ...
        if params.contrastive_loss:
            _,_,z_query = model.extractor(support_set,query_h, support_set.size(0))
            loss += ContrastiveLoss(z_query, map_labels) 
        ...
        for j in range(1, online_epochs):
            ...
            query_set1, query_map_labels1 = utils.shuffle(support_set,map_labels, random_state=j)
            ...
            p, query_h = model.forward_online_QUERY(query_set1, query_h, lwf=True)
            stored_model.eval()
            if (not params.random_stream and counter > 1) or (params.random_stream and counter > N):
            ...
            #running_loss += loss
            if params.contrastive_loss:
                _,_,z_query = model.extractor(query_set1,query_h, support_set.size(0))
        ...
        query_set, query_map_labels = replay_buffer.exemplar_train(excluded_classes)
        print("Replay Buffer: ", np.shape(query_set), np.shape(query_map_labels))
        query_set = torch.from_numpy(np.array(query_set)).float()
        query_map_labels = tf.keras.utils.to_categorical(query_map_labels, num_classes=baseClassesNb + newClassesNb, dtype='int32')
        query_map_labels = torch.from_numpy(np.array(query_map_labels)).float()
        ...
        query_h = tuple([each.data for each in query_h])
        if len(query_set) > 0:
            model.prototype_update_momentum(query_set, query_map_labels, params.alpha, query_h)
        ...
        with torch.no_grad():
            ...
            if newClassesNb > 1:
                ...
                precision, recall, fscore,_ = metrics.precision_recall_fscore_support(true_test_newClasses_oo, test_newClasses_oo, average='weighted')
                try:
                    auc_test = metrics.roc_auc_score(np.vstack(true_test_newClasses_output), np.vstack(test_newClasses_output), average="weighted")
                except ValueError:
                    auc_test = None
                print('Test loss: {}'.format(running_test_loss / n_test_steps))
