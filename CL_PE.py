...
#        query_map_labels = query_map_labels.cuda()
        ...
        log_p, val_h,_ = model.forward_online(support_set,map_labels, support_set, val_h,query_h)
        ...
        if params.prototypical_contrastive_loss:
            _,_,z_query = model.extractor(support_set,query_h, support_set.size(0))
            loss += PrototypicalContrastiveLoss(z_query, map_labels) 
        if params.contrastive_loss_with_prototypes:
            _,_,z_query = model.extractor(support_set,query_h, support_set.size(0))
        ...
        for j in range(1, online_epochs):
            ...
            if params.prototypical_contrastive_loss:
                _,_,z_query = model.extractor(query_set1,query_h, query_set1.size(0))
                loss += PrototypicalContrastiveLoss(z_query, query_map_labels1)   
            #running_loss += loss
            loss.backward()
            optimizer.step()
        #model.online_update_prototypes(support_set,map_labels, val_h)
        ...
        # update prototypes of classes not found in support set using weighted sum of old prototypes and prototypes from replay data (high momentum update set alpha = 0.99)
        model.eval()
        ...
        with torch.no_grad():
            ...
            if newClassesNb > 1:
                ...
                testLoss = {'Allloss_NewClasses': running_test_loss / n_test_steps}
